...
class GitIndexEntry(tp.NamedTuple):
    ...
    def unpack(data: bytes) -> "GitIndexEntry":
        data_lst = list(struct.unpack(f"!10I20sH{len(data) - 62}s", data))
        data_lst[-1] = data_lst[-1].strip(b"\00").decode()
        return GitIndexEntry(*data_lst)
...
def read_index(gitdir: pathlib.Path) -> tp.List[GitIndexEntry]:
    ...
    for i in range(struct.unpack("!I", data[8:12])[0]):
        llp = data.index(b"\00", lp + 62)
        while data[llp] != 0 or (llp - 11) % 8 != 0:
            llp += 1
        result.append(GitIndexEntry.unpack(data[lp: llp + 1]))
        lp = llp + 1
    return result
